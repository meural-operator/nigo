"""
=============================================================================
Full Comparison (Bounded Edition):
FNO, UNet, PINN vs TurboNIGO (Legacy Unbounded) vs TurboNIGO (Bounded+Stable)

All models use the SAME initial condition at ν=0.1 for fair comparison.
The bounded model was trained with:
  - Adaptive Refiner gating (ε_max=0.1, sigmoid-bounded)
  - Spectral Normalization (Lipschitz-1 bounds on all projection layers)
  - 2-Stage Curriculum: 20-epoch MSE warmup → Sobolev H¹ finetuning
  - Near-zero ReZero init (std=1e-6) to avoid spectral_norm singularity
=============================================================================
"""
...
# Legacy TurboNIGO (unbounded) from old run directory
TURBO_DIR = WORKSPACE_ROOT / "RUN_BURGERS_1D"
sys.path.insert(0, str(TURBO_DIR / "src_mirror"))
try:
    from models.turbo_nigo_1d import GlobalTurboNIGO_1D as LegacyTurboNIGO_1D
except ImportError:
    LegacyTurboNIGO_1D = None
    print(f"Warning: Could not import legacy TurboNIGO_1D from {TURBO_DIR}")

# Bounded TurboNIGO (current repo — with adaptive refiner + spectral norm)
sys.path.insert(0, str(REPO_ROOT))
from turbo_nigo.models.turbo_nigo_1d import GlobalTurboNIGO_1D as BoundedTurboNIGO_1D
...
OUTPUT_DIR = REPO_ROOT / "results" / f"BoundedComparison_{ts}"
...
# Bounded model checkpoint (from our robust 2-stage training)
BOUNDED_CKPT_DIR = REPO_ROOT / "results" / "RobustStable_BURGERS_L20_20260403_131253"
BOUNDED_CHECKPOINTS = {
    "Bounded_best":     BOUNDED_CKPT_DIR / "best_model.pth",
    "Bounded_ep120":    BOUNDED_CKPT_DIR / "epoch_120.pth",
}
...
COLORS = {
    ...
    "Bounded_best":      "#00BCD4",    # Teal — high contrast
    "Bounded_ep120":     "#FF5722",    # Deep Orange
}
...
def generate_ic(spatial_resolution, initial_steps, nu, seed=42):
    ...
    ic = np.stack(frames, axis=1)
    ic = ic[np.newaxis, :, :, np.newaxis]
...
def rollout_fno(model, initial_state):
    ...
    for block_idx in range(n_blocks):
        ...
        if diverged:
            last = predictions[-1]
            while len(predictions) < TOTAL_STEPS: predictions.append(last)
            break

    return np.array(predictions[:TOTAL_STEPS]), time.perf_counter() - t_start, diverged
...
def rollout_unet(model, initial_state):
    ...
    for block_idx in range(n_blocks):
        with torch.no_grad():
            for t in range(BLOCK_SIZE):
                inp = xx.reshape(1, S, -1).permute(0, 2, 1)
                pred = model(inp)
                predictions.append(pred.squeeze().cpu().numpy())
                if pred.abs().max().item() > 1e6 or torch.isnan(pred).any():
                    diverged = True; break
                pred_r = pred.permute(0, 2, 1).unsqueeze(-1)
...
# ── Rollout: TurboNIGO (shared for both legacy and bounded) ──────────
def rollout_turbo(model, ic_1024_tensor):
    seq_len = BLOCK_SIZE
    n_blocks = TOTAL_STEPS // seq_len

    u0 = (ic_1024_tensor - G_MIN) / (G_RANGE + 1e-8)
...
def plot_all(results):
    ...
    fig.suptitle(
        ...
        f"PDEBench Baselines vs TurboNIGO (Unbounded) vs TurboNIGO (Bounded+Stable)",
    ...
    for name, res in results.items():
        ...
        lw = 3.0 if "Bounded" in name else (2.5 if "Turbo" in name else 1.8)
    ...
    axes[0].set_ylabel("RMS Energy", fontsize=13); axes[0].legend(fontsize=8, loc="upper right", ncol=2); axes[0].grid(True, alpha=0.3)
    ...
    plt.tight_layout()
    fig.savefig(OUTPUT_DIR / "bounded_diagnostics.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
    print(f"    ✓ bounded_diagnostics.png")
    ...
    for i, (name, res) in enumerate(results.items()):
        ...
        ax.set_title(f"{name}\n(E={res['final_energy']:.4f})", fontsize=7)
        ax.set_xlabel("Time Step", fontsize=8)
        if i == 0: ax.set_ylabel("Spatial Grid", fontsize=9)
        plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)

    plt.tight_layout()
    fig.savefig(OUTPUT_DIR / "bounded_heatmaps.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
    print(f"    ✓ bounded_heatmaps.png")
    ...
    for i, step in enumerate(snap_steps):
        ...
        for name, res in results.items():
            c = COLORS.get(name, "#333333")
            ls = LINESTYLES.get(name, "-")
            lw = 2.5 if "Bounded" in name else (2.0 if "Turbo" in name else 1.2)
    ...
    plt.tight_layout()
    fig.savefig(OUTPUT_DIR / "bounded_snapshots.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
    print(f"    ✓ bounded_snapshots.png")
    ...
    plt.tight_layout()
    fig.savefig(OUTPUT_DIR / "bounded_energy_bars.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
    print(f"    ✓ bounded_energy_bars.png")
...
def main():
    print("=" * 70)
    print("  BOUNDED COMPARISON: PDEBench + Legacy TurboNIGO vs Bounded TurboNIGO")
    print(f"  ν = {NU} | {TOTAL_STEPS} steps | Block size = {BLOCK_SIZE}")
    print(f"  Output → {OUTPUT_DIR}")
    print("=" * 70)

    # ── Generate ICs ──────────────────────────────────────────────
    ic_256 = generate_ic(SPATIAL_RES, INITIAL_STEP, NU)
    ic_1024 = generate_ic(SPATIAL_FULL, 1, NU)
    ic_turbo = ic_1024[:, :, 0, :].permute(0, 2, 1)
    ...
    if pinn_path.exists():
        ...
        print(f"\n  [PINN] NOT FOUND: {pinn_path}")

    # ── Legacy TurboNIGO checkpoints (unbounded) ──────────────────
    for name, ckpt_path in LEGACY_CHECKPOINTS.items():
        if not ckpt_path.exists():
            print(f"\n  [{name}] NOT FOUND: {ckpt_path}")
            continue
        if LegacyTurboNIGO_1D is None:
            print(f"\n  [{name}] SKIPPED: Legacy model class not available")
            continue
        print(f"\n  [{name}] Loading...")
        model = LegacyTurboNIGO_1D(
    ...
    # ── Bounded TurboNIGO checkpoints ─────────────────────────────
    for name, ckpt_path in BOUNDED_CHECKPOINTS.items():
        if not ckpt_path.exists():
            print(f"\n  [{name}] NOT FOUND: {ckpt_path}")
            continue
        print(f"\n  [{name}] Loading (Bounded: adaptive_refiner + spectral_norm)...")
        model = BoundedTurboNIGO_1D(
            latent_dim=64, width=32, in_channels=1, spatial_size=SPATIAL_FULL,
            use_adaptive_refiner=True, use_spectral_norm=True,
        )
        ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False)
        model.load_state_dict(ckpt["model"], strict=True); model.eval()
        ep = ckpt.get('epoch', '?')
        vl = ckpt.get('best_val', float('nan'))
        print(f"    epoch={ep}, val_loss={vl:.2e}, params={sum(p.numel() for p in model.parameters()):,}")
        print(f"  [{name}] Rolling out...")
        traj, t, div = rollout_turbo(model, ic_turbo.clone())
        energy = np.sqrt(np.mean(traj**2, axis=1))
    ...
    for name, r in results.items():
        marker = " ★" if "Bounded" in name else ""
        print(f"  {name:<22} {r['time']:<10.3f} {r['final_energy']:<12.6f} {r['diverged']}{marker}")
    print(f"{'=' * 70}")
    ...
    with open(OUTPUT_DIR / "bounded_comparison.json", "w") as f:
